...
/*   Updated: 2025/10/13 17:39:54 by teatime          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int vlood_vill(int i, int j, t_game *cube)
{
	...
	if (is_a_wall(cube->map->final_grid[i][j]))
		return (0);
	if (cube->map->final_grid[i][j] == 'F')
		return (0);
	// if (cube->map->final_grid[i][j] == ' ' || cube->map->final_grid[i][j] == '0')
	// 	return (-1);
	if (!is_a_valid_char(cube->map->final_grid[i][j], AFTER_FF))
	{
		printf("char qui pose pb : %c et pos i : %d et pos j : %d\n", cube->map->final_grid[i][j], i, j);
		return (-1);
	}
	cube->map->final_grid[i][j] = 'F';
