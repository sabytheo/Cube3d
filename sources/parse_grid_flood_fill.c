...
/*   Updated: 2026/01/06 23:47:28 by teatime          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int flood_fill(int i, int j, t_game *cube)
{
	if (i < 0 || i >= cube->map.grid_height || !cube->map.final_grid[i])
		return (-1);
	if (j < 0 || (size_t)j >= ft_strlen(cube->map.final_grid[i]) || !cube->map.final_grid[i][j] || cube->map.final_grid[i][j] == '\n')
		return (-1);
	...
	if (!cube->map.final_grid[i + 1] || flood_fill(i + 1, j, cube) == -1)
		return (-1);
	if (!cube->map.final_grid[i - 1] || flood_fill(i - 1, j, cube) == -1)
		return (-1);
	if (!cube->map.final_grid[i][j + 1] || flood_fill(i, j + 1, cube) == -1)
		return (-1);
	if (!cube->map.final_grid[i][j - 1] || flood_fill(i, j - 1, cube) == -1)
		return (-1);
	return (0);
