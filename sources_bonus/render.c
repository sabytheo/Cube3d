...
static void	render_ceil(t_cube_thread *cube_thread, float draw_start, int x, float correction)
{
	int		p;
	float	floor_x;
	float	floor_y;
	...
	while (y < draw_start)
	{
		...
		floor_x = cube_thread->cube->player.pos_x + cube_thread->raycast.dir.x * row_distance;
		floor_y = cube_thread->cube->player.pos_y + cube_thread->raycast.dir.y * row_distance;
...
static void	render_floor(t_cube_thread *cube_thread, float draw_end, int x, float correction)
{
	int		p;
	float	floor_x;
	float	floor_y;
...
void	render_textured_floor_ceiling(t_raycast *raycast, t_cube_thread *cube_thread, int x, float draw_start,
		float draw_end)
{
	float	correction;

	correction = cos(raycast->angle - cube_thread->cube->player.angle);
	if (correction < 0.01)
		correction = 0.01;
...
void	render_wall_pixel(t_cube_thread *cube_thread,int x, int *j, float *text_y, t_img *img)
{
	int color;

	color = get_texture_pixel(*text_y, img, cube_thread->textures.x);
	if (color != 16777215)
...
void	render_wall(float wall_height, t_cube_thread *cube_thread, int x, t_raycast *raycast)
{
	...
	cube_thread->textures.y = raycast->texture_assigned->ht / raycast->wall_height;
	...
	if (draw_end >= HEIGHT)
		...
		render_textured_floor_ceiling(raycast, cube_thread, x, draw_start, draw_end);
	j = draw_start;
	while (j < draw_end)
		render_wall_pixel(cube_thread,x,&j,&text_y,raycast->texture_assigned);
}
